...
exports.retrieval = async function() {
    ...
    for (const disease of diseases) {
         ...
         * Pubmed
         */
        const pubmedResult = await http(endpointUtils.pubmedArticleIds(encodeURI(disease)));
        ...
        //const pubDate = xmlUtils.xpathFromXmlString(pubmedResult, '//PubDate/Year');
        for (const articleId of uniqueArticleIds) {
            ...
            let pubDay = xmlUtils.xpathFromXmlString(pubmedArticle, '//PubMedPubDate[@PubStatus!="pmc-release"]/Day');
            let pubMonth = xmlUtils.xpathFromXmlString(pubmedArticle, '//PubMedPubDate[@PubStatus!="pmc-release"]/Month');
            let pubYear = xmlUtils.xpathFromXmlString(pubmedArticle, '//PubMedPubDate[@PubStatus!="pmc-release"]/Year');

            let pubDate = pubYear[0] + "-" + pubMonth[0] + "-" + pubDay[0];
            // MER accepts text with a maximum size of 5940 characters
            if (abstract.length > 0 && abstract[0].length <= 5940 && title.length > 0){
                await pubmedService.saveArticleToDb(articleId, title, abstract, diseaseId, pubDate);
            }
        }
        /**
         * Flickr
         */
        const flickrCallResult = await http(endpointUtils.flickrEndpoint(encodeURI(disease)));
        ...
        for (let i = 0; i < length; i++) {
            ...
            const flickrCallResultDate = await http(endpointUtils.flickrEndpointGetDate(photoIds[i]));
            const flickr_date = xmlUtils.xpathFromXmlString(flickrCallResultDate, '//@taken');
            await flickrService.savePhotoToDb(url, titles[i], flickr_date, photoIds[i], diseaseId);
        }
        /**
         * Twitter
         */
