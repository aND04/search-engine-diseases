...
const saveStrangeDiseasesToDb = async function (disease, medicalSpecialtyId) {
        let sanitizedDescription = stringUtils.sanitize(disease);
        let id = await getDiseaseId(sanitizedDescription);
        if (id === -1) {
            await dbConnector.query(`INSERT INTO dbpedia_disease(description, medical_specialty_id) values ('${sanitizedDescription}',${medicalSpecialtyId})`);
...
const getStrangeDiseaseId = async function() {
    return await dbConnector.query(`SELECT MAX(id) AS lastId from dbpedia_disease`);
};
...
const getDiseaseDescirption = async function(diseaseId){
    const queryResult = await dbConnector.query(`select description from dbpedia_disease where id=${diseaseId}`);
    return queryResult[0] ? queryResult[0].description : -1;
}

module.exports = {
    saveDiseasesToDb: saveDiseasesToDb,
    saveMedicalSpecialtyToDb: saveMedicalSpecialtyToDb,
    saveStrangeDiseasesToDb:saveStrangeDiseasesToDb,
    getStrangeDiseaseId: getStrangeDiseaseId,
    getDiseaseId: getDiseaseId,
    isDiseaseNew: isDiseaseNew,
    getDiseaseDescirption: getDiseaseDescirption,
