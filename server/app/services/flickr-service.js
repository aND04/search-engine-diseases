...
const savePhotoToDb = async function(url, title,flickr_date, flickr_id, diseaseId) {
    const id = await getPhotoId(flickr_id);
    if (id === -1) {
        const localTitle = await stringUtils.encodeBase64(title);
        const queryResult = await dbConnector.query(`INSERT INTO flickr_photo(flickr_id, url, title, flickr_date) VALUES (${flickr_id}, '${url}', '${localTitle}', '${flickr_date}')`);
