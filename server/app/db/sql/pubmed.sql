...
create table relevance (
  ...
  dbpedia_disease_id bigint(20) not null,
  tfidf float (12,11),
  similarity float(12,11),
  explicitFeedbackValue float(12,11),
  implicitFeedbackValue float(12,11),
  date_Relevance float(12,11),
  relevance_Avg float(12,11),
  primary key (pubmed_article_id, dbpedia_disease_id),
  constraint relevance_pubmed_article_id foreign key (pubmed_article_id) references pubmed_article (id),
  constraint relevance_dbpedia_disease_id foreign key (dbpedia_disease_id) references dbpedia_disease (id)
);